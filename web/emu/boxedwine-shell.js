 ...
 */
            ...
            if(allowParameterOverride()){
                if(property.length > 6) {
                	if( (property.startsWith("%22") && property.endsWith("%22") )
                    	...
                    	return kv[0].trim() + "=" + kv[1].trim(); // bare KEY=VALUE for Boxedwine -env
                	}else{
        ...
        function loadFile(pathPrefix, filename, callback) {
			let settled = false;
			let finish = function(bytes, error) {
				if (settled) return;
				settled = true;
				callback(bytes, error);
			};
			if (typeof globalThis.boxedwineTakeEmbeddedFile === 'function') {
				if (statusElement) statusElement.textContent = 'Loading ' + filename + '...';
				let embedded = globalThis.boxedwineTakeEmbeddedFile(filename);
				if (embedded) {
					Promise.resolve().then(function() { finish(embedded, null); });
					return;
				}
			}
			fetch(pathPrefix + filename, { method: 'GET' }).then(function(response) {
       			if (response.status === 200) {
					...
					var chunks = [];
					var loaded = 0;
					return reader.read().then(function pump(part) {
						if (part.done) {
						...
						if (statusElement) {
							var suffix = total ? ' ' + Math.min(100, Math.floor(loaded * 100 / total)) + '%' : '';
							statusElement.textContent = 'Downloading ' + filename + suffix;
						}
						if (progressElement && total) {
				...
				if (!buffer) throw new Error('Unable to load ' + filename + ': empty response');
				finish(buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer), null);
			}).catch(function(error) {
				console.error(error);
				finish(null, error);
			});
		}
		function reportFileSystemError(error) {
			console.error('BoxedWine filesystem setup failed', error);
			if (statusElement) statusElement.textContent = 'Unable to start: ' + error.message;
        ...
        function buildAppFileSystem(callback) {
            if(Config.appPayload.length > 0){
				...
				loadFile(Config.locateAppBaseUrl, Config.appZipFile, (uint8Array, error) => {
        ...
        function buildExtraFileSystems(callback) {
            ...
            if(Config.extraPayload.length > 0){
                ...
                for(let i = 0; i < Config.extraZipFiles.length; i++) {
					loadFile(Config.locateOverlayBaseUrl, Config.extraZipFiles[i], (uint8Array, error) => {
        ...
        function initBrowserFilesystem(callback) {
			...
			if (Config.storageMode == STORAGE_INDEXED_DB) {
  				...
  				FS.syncfs(true, function (err) {
  					if (err) {
	  					console.warn('unable to sync persistent folder; continuing in this session: ' + ROOT, err);
	  				}
	  				callback();
				});
			} else {
        ...
        var initialSetup = function(){
			...
			buildExtraFileSystems((extraError) => {
				if (extraError) { reportFileSystemError(extraError); return; }
				buildAppFileSystem((appError) => {
					if (appError) { reportFileSystemError(appError); return; }
					loadFile(Config.locateRootBaseUrl, Config.rootZipFile, (rootZipfileBytes, rootError) => {
						if (rootError) { reportFileSystemError(rootError); return; }
      ...
      var Module = {
        // Standalone packaging defines this decoder before the shell. Feeding
        // wasmBinary prevents Emscripten from issuing an external .wasm fetch.
        wasmBinary: typeof globalThis.boxedwineTakeEmbeddedFile === 'function'
          ? globalThis.boxedwineTakeEmbeddedFile('boxedwine.wasm')
          : undefined,
        preRun: [initialSetup],
        arguments: [],
        postRun: [],
        ...
        print: (function() {
          ...
          return function(text) {
            ...
            if (text.indexOf('Setting PersistentGameData ReadOnly to True') !== -1 &&
                typeof Module._wasm_jit_enable_execution === 'function') {
              Module._wasm_jit_enable_execution();
              Module.boxedwineDirectCanvas = true;
              document.documentElement.classList.add('boxedwine-direct-canvas');
            }
      ...
      // Make the shell configuration explicit for inlined standalone builds;
      // a later classic-script `var Module` declaration must reuse this object.
      ...
      window.alert = function(msg) {
          console.error('[alert suppressed]', msg);
          try {
              if (String(msg).indexOf('getPage unknown type') >= 0 &&
                      Module._wasm_jit_debug_last_call_ptr && Module.HEAPU32) {
                  var ptr = Module._wasm_jit_debug_last_call_ptr() >>> 2;
                  console.error('[WASM JIT alert breadcrumb]', JSON.stringify({
                      tableIndex: Module.HEAPU32[ptr] | 0,
...
function initWasmJitCache(callback) {
    ...
    var loadServerCache = function() {
        if (getParameter("jit-cache") === "false") {
            console.log('[WASM JIT] server cache disabled by query');
            done();
            return;
        }
        fetchServerJitCache(done);
    };
    var cacheLoadStarted = false;
    var loadServerCacheOnce = function() {
    ...
    try {
        if (typeof indexedDB === 'undefined') {
            throw new Error('IndexedDB is not exposed by this browser context');
        }
        req = indexedDB.open('boxedwine-wasm-jit', 1);
    } catch (error) {
        console.warn('[WASM JIT] IndexedDB unavailable; using bundled JIT cache in memory', error);
        loadServerCacheOnce();
        return;
    }
    ...
    req.onerror = function() {
        console.warn('[WASM JIT] IndexedDB unavailable; using bundled JIT cache in memory');
        loadServerCacheOnce();
    };
    req.onsuccess = function(e) {
        ...
        try {
            ...
            tx.oncomplete = loadServerCacheOnce;
            tx.onerror = loadServerCacheOnce;
            tx.onabort = loadServerCacheOnce;
        } catch (error) {
            console.warn('[WASM JIT] IndexedDB transaction failed; using bundled JIT cache in memory', error);
            loadServerCacheOnce();
        }
    };
}
...
function updateSaveJitButton() {
    if (Config.showSaveJITCache && Config.recordJITCache === true &&
        ...
        var button = document.getElementById('savejitbtn');
...
function fetchServerJitCache(callback) {
    ...
    var filename = getJitCacheZipFilename();
    if (typeof globalThis.boxedwineTakeEmbeddedFile === 'function') {
        var embedded = globalThis.boxedwineTakeEmbeddedFile(filename);
        if (embedded) {
            importJitModulesFromBuffer(embedded).then(callback).catch(function() { callback(); });
            return;
        }
    }
...
async function saveJitModules() {
    ...
    try {
        ...
        var storeOnly = false;
        try {
            storeOnly = new URLSearchParams(globalThis.location.search)
                .get('jit-export-stored') === 'true';
        } catch (e) {}
        ...
        var compressed = await Promise.all(cacheEntries.map(async function(kv) {
            ...
            var compData = storeOnly ? data : await deflateRaw(data);
            return { name: key + '.wasm', data: data, compData: compData,
                method: storeOnly ? 0 : 8, crc: crc32(data) };
        }));
        ...
        if (Module.wasmJitBlockMeta && Module.wasmJitBlockMeta.size > 0) {
            ...
            compressed.push({
                ...
                data: manifestData,
                compData: storeOnly ? manifestData : await deflateRaw(manifestData),
                method: storeOnly ? 0 : 8,
        ...
        if (Module.wasmJitInteriorProfileLines && Module.wasmJitInteriorProfileLines.length > 0) {
            ...
            compressed.push({
                ...
                data: profileData,
                compData: storeOnly ? profileData : await deflateRaw(profileData),
                method: storeOnly ? 0 : 8,
        ...
        compressed.forEach(function(e) {
            ...
            lh.setUint16(8,  e.method,   true); // STORED or DEFLATE
            ...
            cd.setUint16(10, e.method,   true); // STORED or DEFLATE
