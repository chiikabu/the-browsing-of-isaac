...
"live numbers" block in it — including the 2026-08-20 one preserved below —
goes stale within days and has repeatedly cost units that trusted it. The live
state is derived from the tree:

```
node scripts/decomp/status.mjs     # families, open boundaries, verification
```

and the current hand-off pointer lives in `decomp/frontier.json`. Censuses go
through `python scripts/decomp/tools/pequery.py` (prebuilt PE index).

Historical snapshot (2026-08-20, STALE — kept for the removal trail):
...
| | value |
|---|---|
| recomp boot round 10b (2026-09-01) | **The msvcp140 C++ iostream layer, and a general baked-purge bug the lifter had.** After round 10 the boot stopped at `msvcp140!basic_ios<char>::basic_ios()`. `scripts/recomp/host/src/host_shims_msvcp.c` now re-implements the streambuf / basic_ios / istream / ostream / iostream members the game reaches, on the **real MSVC x86 object layout** (transcribed from `C:\Windows\SysWOW64\msvcp140.dll` 14.44; abi-notes at `output/decomp/_scratch/msvcp140/abi-notes.md`). It is not a call into the DLL: the game's own `basic_stringbuf` overrides are lifted (vtable `0xb1b190`) and it manipulates the objects' fields directly, so every shim locates the `basic_ios` vbase as `this + [[this]+4]` (never a constant), touches the streambuf only through its six indirection pointers, and dispatches the streambuf virtuals (overflow +0xc, underflow +0x18, uflow +0x1c, sync +0x34) through the object's real vtable into lifted code (recognising the base defaults by their IAT jump thunks `0x00aef065..0x00aef0a1`). Proven by **24 new host-selftest checks (103 → 127)** over a synthetic stringstream on the game's layout — construct / `_Ipfx` skip / `operator>>` parse / `sgetc`-`sbumpc`-`snextc` / empty-get-area uflow dispatch / `operator<<` hex+width+fill / `write`+`put` / full-put-area overflow / `flush` / three-vtable destruction — and **mutation-checked** (sgetc ignoring the count, `>>` dropping eofbit, `<<` keeping width, the iostream vtordisp) through `scripts/decomp/mutate.mjs`. **The general bug:** the lifter bakes each direct host-import call's stack purge INTO THE CALLER at lift time (`imp_X(s); s->EIP=MEMR32(s->ESP); s->ESP += 4u + <purge>u;`), read from the shim table. `gen_shims.py`'s push-count sweep miscounts `__thiscall` ctors whose args are set inline at the caller — it read `basic_ios()` (`@@XZ`, 0 args) as purge **32**, plus iostream 8→4, ostream 12→8, `_Lockit` 36→4, `widen` 12→4 — so the lifted `std::stringstream` ctor over-popped 32 bytes and passed `sb == 1`. Fixed WITHOUT re-lifting the 272 MB image: `scripts/recomp/lift/lift_patches.py` gained `PURGE_PATCHES`, which rewrites the baked constant in place at each of the 12 call sites and drops the touched TU's object (like the `0x009ab970` prologue patch); `build_boot.py` recompiled only 4 TUs. **Standing rule:** correcting a purge in `gen_shims.py` requires adding its `(wrong,right)` to `PURGE_PATCHES` (or a re-lift); `lift_patches.py --check` lists stale sites. With that, construction and the first `operator>>` parse are correct (traced: `sb`/`_Mystrbuf` = the real streambuf). **Next fault (lifter side, documented not fixed):** the boot faults just after that `operator>>` returns, in tokenizer `0x0067f420` at `0x0040cf50`, because the caller's `esi` is a misaligned `0x0dfc4b2f` — a callee-saved register was clobbered. An ESP/register trace (logged at `_Ipfx`/`_Unlock`) proves `operator>>` is stack-balanced, so the clobber is UPSTREAM in the stringstream ctor `0x00684ce0` or `0x0067f420`: one of their calls does not preserve `esi`/`edi`/`ebx`. Next: a callee-saved-register preservation trace across `0x00684ce0`'s call sites. (An earlier draft blamed the `_Lock`/`_Unlock` tail-jump thunks; disproven — that path is balanced.) Selftest 127/0, `tests/recomp-host.test.js` 33/33, decomp preflight PASS. Docs: recomp-architecture.md §20, recomp-boot.md, HANDOFF.md, frontier.json. |
| recomp boot round 10 (2026-09-01) | **Archive/asset wall down; boot reaches the C++ iostream runtime.** The round-9 "archives never fopen'ed" blocker had three parts, none in the archive code. (1) KAGE builds each mount root's `std::map` index by a **directory scan** (`0x00a710a0` → `0x00a15f10` → vtable+8 → `0x00a687f0` Scan → readdir `0x00a172e0` → opendir `0x00a16f50`): `mbstowcs_s → GetFullPathNameW(size query) → malloc → GetFullPathNameW → "\*" → FindFirstFileW → FindNextFileW` (register-held import, `mov edx,[0xb1825c]` @0x00a17369 — invisible to the call-site census, hence "never called" + unknown purge) → `wcstombs_s`. Fixed in `host_shims_fs.c`/`host_shims_conv.c`: `GetFullPathNameW` returns the required WCHAR count incl. terminator on the size query and too-small buffers (was 0 → opendir failed before any Find call), real `FindFirstFileW`/`FindNextFileW` over the RAM-FS (WIN32_FIND_DATAW layout, attr/size/alt-name), real `wcstombs_s` (pReturnValue counts the terminator; readdir stores ret-1); `gen_shims.py` curated purge 8 + PROVIDED for the pair. (2) The install is not only archives: this instance's `animations.a` has TOC count 1 (`resources/animations.b`, a 4 MB type-1 bundle); the `.anm2`/shader/Lua/xml files are 476 **loose** files under `resources/` (38 MB) that the scan indexes and the resolver serves by path — never seeded. `boot_integration.mjs` seeds the whole tree (+ sfx.a); RAM-FS 512 → 8192 slots, per-scan cap 128 → 2048 (`gfx/ui` has 143 children). Archive members are keyed by FULL path (`djb2("resources/gfx/ui/pausescreen.png")` = graphics.a entry 1835, hash2 = FNV-1a `0x00a15ab0` of the same string; 122 loose files match TOC rows 1:1). (3) `0x009ab970`, the function that creates the `resources/` root, is `xor eax,eax; ret` in the canonical exe — **one of this project's own 19 emulator-era patch runs (172 B; pristine `55 8b ec` in `tools/isaac-ng.unpacked.exe.pre-coinit`)**; the AnmCache (`0x0040db90`) prefixes each root NAME to `gfx/ui/x.anm2`, so with only the `""` root nothing resolved. Undone by a re-applicable **lift patch** (`scripts/recomp/lift/lift_patches.py`: `build_boot.py` rewrites the generated `sub_009ab970` to emulate the prologue and enter the lifted orphan `sub_009ab973`; one TU recompiles; canonical hash untouched; the full patch table is in recomp-architecture.md §19.5). **Result:** 2 mount roots, `[0x00c37b14]=6` archives, **0 `Could not open`** (from 33+), every shader initialises (Water/Mirror/Heat Wave/Hallucination/Dizzy… were "Failed to load vertex shader"), renderbuffers allocate, OpenAL/theoraplayer init, `enums.lua`+`main.lua` run; new stop = `msvcp140.dll!basic_ios<char>::basic_ios()` @`0x00684d24` in `0x00684ce0` (`std::stringstream(const string&)`) from the whitespace tokenizer `0x0067f420` (41 callers). **Verification:** host selftest 82 → **103 checks, 0 failures** (wide scan chain, `GetFullPathNameW` two-call protocol, 700-file capacity), 5 mutants through `mutate.mjs` (dir attribute, terminator count, Next never advances, size query → 0, cap 128) all KILLED; `tests/recomp-host.test.js` 34/34; the boot trace is the run itself. **Speed:** `build_boot.py` now links `-O0` by default — link **474 s → 8.0 s**, host-only relink **520 s → 133 s** (`--opt-link` for shipping); `--fast-link`/`--no-lift-patches` flags. The boot must run with cwd = the instance dir (host Lua's libc is `-sNODERAWFS`). Scout: the only `CreateThread` is the theoraplayer worker `0x00aab120`; nothing on the init chain waits on it. |
| harness round 3 + ABI 101 recovery (2026-09-01) | **The idx-3 leaf-5 unit was found stranded and the flow that let it strand was fixed.** The 2026-08-31 session had landed the idx 3 (`0x0092f1c0`) case-1 leaf-5 `0x74f090` fold as **Update ABI 100→101** (runtime `+opaque0092f1c0GameType0` @23696, 23696→23700; `92f1c0_try_pure` gains `game_type_0`/`flags_2654c`; leaf 5 hosts only when AL==0) but never committed: the cpp on disk carried the hand-applied mutation `== 0u /* MUTANT: inverted verdict */` (the mutant build had been re-linked at 23:11 after the 23:10 passing differential), the JSON had been re-serialized at indent 2 (a 74,117-line diff for a 3-key change), its `runtimeInputs` rows lacked the new lane, frontier.json/port log were not updated, and the slice suite was **93-red** on 72 literal `spec.abiVersion, 100` pins + 10 `ABI_VERSION, 100` + 11 `23696` size pins. Recovery: polarity restored against the instruction stream (`0x0092fe66 test al,al; 0x0092fe68 jne 0x92ff60` = AL!=0 → shared pure tail; `0x0074f093 cmp eax,5 / ja` after `dec` = unsigned (mode-1)<=5 window; `0x0074f0a5 and ecx,0x10000`), mutant re-run through the new wrapper (differential mismatch at case 143: `opaqueCall0092f1c0` expected 0 actual 1, `continuationKind` 7 vs 20 — MUTANT KILLED), rebuilt, **differential 5392 cases pass @ ABI 101 with `leaf5=2p/4h`** (both 0x74f090 verdicts exercised; the corpus draws `flags2654c` in both bit-16 polarities and small limits so counter==limit-1 is reachable), slice suite **864/864**, `npm test` **3894/3894** (80 s). **Harness fixes (each a measured failure class):** (1) `scripts/decomp/lib/consistency.mjs` — shared tree-consistency checks run by `status.mjs` (ERRORS/WARNINGS), `verify-unit.mjs` stage 0 and `tests/decomp-toolkit.test.js`: header/model/JSON ABI agreement per family, cpp `sizeof` pins vs `ABI_SIZES`, JSON layout drift + canonical form, stranded mutants (`/* MUTANT */` markers, `.mutation-journal.jsonl`, leftover `.bak`), literal ABI pins in tests, frontier freshness, uncommitted unit files. (2) `scripts/decomp/slice-json.mjs check\|fmt\|sync` — the only writer of `decomp/game-update-slice.json` (indent 1 = the committed form); `sync` mirrors `RUNTIME_INPUTS_LAYOUT`/`EVENTS_LAYOUT` into the rows and found the spec had silently drifted: **139 lanes with no row, 255 rows with stale offsets** (the whole v132 B16 hit block described an earlier draft: JSON `waterB16WaterHitCellIdx1..8` sequential @15760 vs layout CellIdx/EntPtr interleaved with Water/Lava aliased), **9 events missing** — all back-filled with an `abiV101` provenance note. (3) `scripts/decomp/mutate.mjs --file --from --to -- <cmd>` — crash-safe mutation runner (stash + journal + `/* MUTANT */` tag + sha256-verified restore in finally/signal handlers; `check`/`restore` for a stranded trail); AGENTS.md now bans hand-edited mutants. (4) `verify-unit.mjs` restaged: preflight (≈1 s, fails before any 15-min suite) → slice build once + `abi.json` zero-imports/ABI check (the old gate never checked abi.json and never built) → **all remaining gates in parallel** (differential ~3.5 min is the wall time; the old gate ran them serially), family suites auto-detected from the dirty tree, `--handoff` = strict preflight + `npm test`. (5) `verify-game-update-slice.mjs` stale-wasm guard now also compares the module against the SOURCES (it compared only against the build script, so a mutant wasm newer than the script was verified against a restored cpp). (6) `pequery.py fieldrefs DISP [FUNC]` + index decode config **v2** (`fld` table: every `[reg+disp]` operand with base≠esp/ebp, r/w/rw/addr + base register + size; 342,708 rows, build 48 s, 88→103 MB) — the object-field census the absolute-displacement xrefs could not do; e.g. `fieldrefs 0x2654c 0x74f090` = exactly the 1 predicate read, `fieldrefs 0x1ba84` = 3 byte writers whole-.text (0x6fa300/0x6fdf72/0x959a1e, none in 0x92f1c0). `verify` ground truths unchanged. UTF-8 stdout + broken-pipe tolerance. (7) `brief.mjs` prints **live-bridge lane delivery** per boundary — and exposed that `web/js/native-update-bridge.js` delivers **0 of the 6 `opaque0092f1c0*` lanes**: the whole 92f1c0 try_pure ladder (v86→v101, this unit included) is DORMANT in the shipped tick; wiring that capture is the recommended next Front-A unit (frontier.json). (8) **All 15 family suites swept to symbolic ABI pins**: 700+ `assert.equal(<FAMILY>_ABI_VERSION, N)` / `abi(), N` literals → `HEADER_ABI_VERSION` parsed from the family header (model-vs-header agreement is the assertion). Runbook/AGENTS/agents updated; frontier.json at ABI 101. |
| harness + perf round 2 (2026-08-31) | **Token-lean unit harness + the two remaining hot spots.** (1) **Family-suite wasm build cache** (`tests/wasm-build-cache.mjs` + an 18-file mechanical wrap of every suite's `buildWasm`): clang+em++ spawns keyed by sha256(cpp+header+EXPORTS+tag); mutants hash differently so mutation checks still really build, restores hit the cache. Measured: room suite **8m46s cold → 11.2s warm (47×)**; FULL `npm test` **~37 min → 1m17s fully warm (3888/3888)**; `ISAAC_WASM_BUILD_CACHE=0` opts out; cache under `output/decomp/wasm-build-cache/`. (2) **Fused runtime writer** `writeRuntimeInputsForLayout` (game-update-model.mjs): the live tick no longer materializes the 4208-key normalized object per write — same precedence chain (menu-aux lanes always win incl. isnil=1 defaults, timer lanes win only in array form, caller keys, door-slot flatten, packed zero) resolved per lane tag into the SAME `writeField`; byte-equivalence vs normalize+writeStruct pinned over 8 adversarial input shapes (flat-key-vs-lane precedence both directions, string anm2 names, undefined/null, randomized key subsets) in tests/decomp-frame-path-fastpath.test.js; all 6 frame-path call sites swapped. Measured: node session tick fast **5.6 → 0.76 ms** (lockstep 7.0 → 2.5), live browser Update tick **4.9 → 2.25 ms** — cumulative from the session start **18.1 → 2.25 ms (8×)**, render 0.89 ms, ~5× headroom under the 16.7 ms budget. 120-tick lockstep-vs-fast differential still 0 mismatches, hostTotals identical; frame-path/bridge/state-patch suites 82/82. (3) **Agent workflow harness**: `scripts/decomp/brief.mjs <VA\|idx>` — one-call unit orientation (boundary status from the slice JSON + function shape from the PE index via node:sqlite + next commands); `pequery.py batch "q ;; q ;; ..."` — N censuses in one process/tool call; `scripts/decomp/verify-unit.mjs` (npm run decomp:gate) — the whole AGENTS.md verification block in one call with one PASS/FAIL line per gate, first-failure excerpts only, and the emsdk env set up internally (the measured 82-phantom-failure class); `.claude/agents/decomp-scout.md` (read-only evidence gatherer, fixed ≤60-line report) + `.claude/agents/decomp-unit.md` (full unit executor with compressed runbook contract, fixed hand-off format). Runbook/AGENTS.md updated to route through these. Gate run post-change: slice+pipeline 857/857, differential 5387 pass, safety pass, diff clean. |
| tooling + fast path (2026-08-31) | **Session-orientation toolkit + shipped-tick fast driver.** (1) `scripts/decomp/status.mjs` derives the live state from the tree (family ABI greps + slice JSON + frontier.json + build artifacts) — doc checkpoint narratives are no longer the orientation path; `decomp/frontier.json` is the hand-off pointer every unit updates. (2) `scripts/decomp/tools/` PE toolkit: `build-pe-index.py` one-time full linear decode (capstone skipdata resync; 763 dword jump tables byte-scanned, validated ≥2 targets, masked + recorded as `jtab` xrefs; 2,082,541 insns, 373 resync bytes; decode config `linear+skipdata+jtab-mask v1`) into SQLite (88 MB, ~50 s) with mem_r/mem_w/mem_rw(+_idx scaled-index) absolute-displacement xrefs via capstone access bits, rel32 call/jmp/jcc edges, `call [slot]` build-time resolution (257), IAT import map (622), int3+call-target function segmentation (19,038 starts), .rdata/.data strings; `pequery.py` census CLI (writers/readers/xrefs-to/callers/callees/body/disasm/sig/addr/strings/imports/meta) + `verify` replaying AGENTS.md ground truths — 0xc7169c writers {0x009aaab0, 0x009ab8cc} exact, 0x40c7f0 73 direct callers exact, 0x74d4a0 1+0 exact, 0xc79100/0xc7b640 zero-writer confirmed, 0x8068f0 = 225-insn body / 1 direct caller (byte-scan-confirmed; the old "19 callsites" note was body-internal callsites). Census scripts are now BANNED in favor of pequery (AGENTS.md session-start block; AGENTS.md slimmed 42→15 KB with v65–v82 narratives archived verbatim to docs/decomp-history.md). (3) **Fast driver `runNativeGameUpdateTickWasmOnly`** (frame-path.mjs): the shipped browser tick no longer steps the JS oracle in lockstep every rAF — measured 15.9 ms/tick session cost in-browser (26 ms node) with the wasm step at ~1 µs; the fast driver replicates the lockstep continuation chain reading `continuationKind` from the wasm events struct, same recapture seams (drainPatches/recaptureAfter before each resume), same one-path F4 post-Pass-A recount ladder. `lockstep = true` stays the default for every API (tests/legacy callers unchanged); the browser bridge opts into fast with `?lockstep=1` as the debug escape hatch. Hot-path model fixes with identical semantics: layout-entries WeakMap cache in read/writeStruct (was re-materializing 4208 `Object.entries` pairs per struct op), frozen zero-template for `normalizeRuntimeInputsForLayout` (was rebuilding + ~6× copying a 4208-key object per tick, ~10 ms), in-place menu-aux/timer lane assigners (delegating copying wrappers keep the exported helpers' contracts). Equivalence pinned by tests/decomp-frame-path-fastpath.test.js: 120 patched/runtime-varied ticks, byte-identical state/events/continuations/hostTotals lockstep-vs-fast; mutant (readKind→-999, kills all resumes) FAILS the differential and was restored (recapture seam not exercised by this corpus — bridge B8 is the only live recaptureAfter consumer, noted honestly). Measured in the live page: Update tick **18.1 → 4.9 ms** (render 0.74 ms; 60 fps budget 16.7 ms now has headroom; pre-fix the Update tick alone busted it at ~55 tps ceiling). Slice+pipeline suites 857/857 green post-edit. (4) **Room family suite repaired 82 fail → 155/155 green** — the failures predate this unit (left by the died-mid-landing v88/v89 units): 72 were hardcoded `assert.equal(abi(), 87)`-style pins that every past ABI bump hand-sedded to the then-current number and the v88 bump missed (stale "wasm ABI 75"/"(74)" message strings prove the sed history); ALL numeric abi() pins are now `ROOM_PURE_ABI_VERSION` (112 sites). The remaining 3 exposed a REAL defect-class-9 bug in the landed v89 deco lane: `isaac_room_deco_render_target` inverted the `0x99ab0c comiss/jbe` gate (`if (!(probe <= 0.0f)) return` — skips on ordered `probe > 0`, the exact opposite of the machine). Re-confirmed from the instruction stream via `pequery.py disasm 0x99ab00` (comiss xmm0,xmm1 / jbe 0x99ab83 = skip on <=0-or-NaN, setter 0x7b9940 @0x99ab1d on the fall-through) and against the independent JS oracle `roomDecoRenderTarget` (which was correct); fixed to `if (!(probe > 0.0f)) return` (NaN still skips, matching unordered jbe). v89 fixed-edge + LCG-differential + mutation-check tests all green after the one-line fix. Also: `.git/index.lock` was stale since 2026-08-21 (zero git processes) — git was silently unusable for 10 days, which is why the whole port tree sat uncommitted; removed. (5) **Bridge capture packs implemented for the specced-but-never-landed §W31/v139 seams** — tests/decomp-frame-input-bridge.test.js carried complete specs (torn-read/over-cap/arm matrices) for four producers that did not exist; the suite was 9-red. Landed in web/js/native-update-bridge.js following the captureUpdateB8/TailPath house pattern (all-or-nothing groups, READ-ONLY guest sampling, absence = byte-for-byte monolith): `captureUpdateGreedProbe` (player-vector walk [gp+0x1baa8]/[+0x1baac], cap 8, per-row f2c/f3bc/161c/parent-1e68/20a9-low-byte, parent_161c read ONLY under a live parent ptr, parent_is_self pointer equality), `captureUpdateB16Blob` (MSVC action-map presence walk from [Game+0x1bbd8] for keys {0x38,0x39,0x40,0x46} — node _Left+0/_Parent+4/_Right+8/_Isnil+0xd/key+0x10; gate39 = mode!=0x2c && SIGNED-odd stage <8 && diff 0; 0x46 REPLACES maskB with 0x7f per cmovne 0x6f96ec; Room src vector +0x1b10..+0x1b1c,+0x1b24,+0x1b28 skipping +0x1b20; desc +0x44/+0x54; reused echoes ambientRoom1d18/b3b7WidthC/b3b7Height10 in the same group), `captureUpdateB16SpawnRows` (+ exported UPDATE_CAPTURE_B16_SPAWN_ROOM_OFFSETS; interior type-7 enumeration outer-x/inner-y, gated on roomB16NeedsType7Grid/NeedsLavaBandGrid module truth with lavaPost derived via the 0x3f6b851f decay; both-closed = zero pack with NO grid scan; cap 64), `captureUpdateB1RainRows` (v139 idx13/idx35: desc [+0x58] seed chain, TWO xorshift steps per row, corner pick/world-xy via the landed roomB1 laws, count = unsigned min(4/6/8 seam base, decor 0x78-stride count), zero-seed fatal → EMPTY patch, host-domain outcome lanes explicit 0), all wired into `captureUpdateLanes` (new b16ArmState/b1RainOwnerState params; per-pack group independence pinned by the lanes tests). Bridge suite 21/21; gate39 parity mutant killed (2 tests) and restored green. Frame-input-bridge VK pin updated for the landed movement keys (KeyW → 0x57; KeyQ pins the still-unmapped arm). web/js/app.js in-run sprite resolver now resolves each path segment case-insensitively against MEMFS (mount preserves the archive's inconsistent casing — gfx/Monsters vs gfx/effects; uploads went 2→8); web/game + web/proto harnesses fetch the served `/@decomp/wasm/game-update-slice.wasm` instead of requiring binary copies in web/ (which tripped check-repo-safety); repo-root scratch (200+ files) atticked to `.scratch/attic-20260831/` with .gitignore blocks so future sessions cannot litter the root; root `section-notes/`+`cpu-dump/` merged into canonical `output/decomp/5129df723e64/`. |
